...
# Abrindo arquivo para armazenar a Lista de Tokens para testes da Analise Sintatica
...
def start(atual, prox):
    ...
    if atual[0] == 'registro':
        ...
        erros_sintaticos.append("Erro: Uma producao inicial (registro, constantes, variaveis, funcao ou algoritmo) era esperada, linha %d" % atual[2])
...
def registro_declaracao(atual, prox):
    ...
    if  controle == 0:
        if atual[1] == 1:                       # Aqui ja usa como paramentro o tipo do token
            ...
            erros_sintaticos.append("Erro: Um identificador era esperado, linha %d" % atual[2])
            controle = 1
            ...
            if atual[0] == '{' and (prox[0] in tipo_primitivo or prox[1] == 1):
                ...
                erros_sintaticos.append("Erro: Um tipo primitivo era esperado, linha %d" % atual[2])
                erros_sintaticos.append("Erro: Um identificador era esperado, linha %d" % atual[2])
                pilha.append(atual)
                tipo_producao[0] = 'declaracao_reg'
                controle = 3
                ...
                erros_sintaticos.append("Erro: Uma '}' era esperada, linha %d" % atual[2])
                pilha.append(atual)
                tipo_producao[0] = 'declaracao_reg'
                ...
                if prox[0] in tipo_primitivo:
                    ...
                    erros_sintaticos.append("Erro: Uma '}' era esperada, linha %d" % atual[2])
                    erros_sintaticos.append("Erro: Token incorreto %s, linha %d" % atual[0] % atual[2])
        ...
        erros_sintaticos.append("Erro: Na declaracao do registro, linha %d" % atual[2])
...
def declaracao_reg(atual, prox):
    ...
    if atual[0] in tipo_primitivo and controle == 1:
        ...
        if atual[0] == ';':
            ...
            erros_sintaticos.append("Erro: Um ';' era esperado, linha %d" % atual[2])
            tipo_producao[0] = 'declaracao'
            tipo_producao[1] = 'declaracao_reg'
            controle = 1
            declaracao(atual,prox)
...
def declaracao(atual, prox):
    ...
    if atual[0] in tipo_primitivo and controle == 1:
        ...
        erros_sintaticos.append("Erro: Na declaracao, linha %d" % atual[2])
...
def constantes_declaracao(atual, prox):
    ...
    if controle == 0:
        ...
        if atual[0] == '}':
            ...
            erros_sintaticos.append("Erro: Uma '}' era esperada, linha %d" % atual[2]) #PRIMEIRO ERRO AQUI!!!
            tipo_producao[0] = 'start'
            controle = 0
            start(atual,prox)

    else:
        erros_sintaticos.append("Erro: Na declaracao das constantes, linha %d" % atual[2])
...
def declaracao_const(atual, prox):
    ...
    if controle == 1:
        ...
        if atual[0] in tipo_primitivo:
            ...
            erros_sintaticos.append("Erro: Um identificador era esperado, linha %d" % atual[2])
            erros_sintaticos.append("Erro: Um '=' era esperado, linha %d" % atual[2])
            erros_sintaticos.append("Erro: Um valor primitivo era esperado, linha %d" % atual[2])
            pilha.append(atual)
...
################################## -----------------------------------ANALISAR AQUIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII'?'''''''''''''''''''''''''''-----------------------------------
        ...
        if atual[0] == ';':
            ...
            if prox[0] in tipo_primitivo or prox[1] == 1 or prox[0] == '=' or isvalorprimitivo(prox) or prox[0] == ';':
                tipo_producao[0] = 'declaracao'
                tipo_producao[1] = 'declaracao_const'
                controle = 1
...
def declaracao_var(atual, prox):
    ...
    # ou vou para producao 23: token_identificador token_identificador; <declaracao_var>
        ...
        tipo_producao[1] = 'declaracao_var'
        ...
        erros_sintaticos.append("Erro: Na declaracao de variaveis, linha %d" % atual[2])
...
def matriz(atual, prox):
    ...
    if atual[0] == '[' and controle == 4:
        ...
        if atual[0] == ']':
            ...
            # ESSA PARTE AQUI EMBAIXO EU COLOQUEI PQ VI LÁ EM CIMA, MAS NÃO SEI SE TÁ CERTA E SE É ISSO AÍ MESMO!!!!!!!!!!!!!!

            # Significa que veio de <fator> em seguida voltar para <exp_simples> e deve vir <fator_deriva> em seguida
            elif (atual[0] == '*' or atual[0] == '/' or atual[0] == '+'  or atual[0] == '-' or atual[0] in op_relacional) and (controle_comando[iterador] == 4 and pilha_comandos.pop() == 'fator'):
                controle = 7
...
def funcao_declaracao(atual, prox):
    ...
    if controle == 0:
        ...
        if (atual[0] in tipo_primitivo or atual[0] == 'vazio'):
            ...
            erros_sintaticos.append("Erro: O tipo de retorno da funcao era esperado, linha %d" % atual[2])
            controle = 1
        ...
        if atual[0] == '{':
            ...
            erros_sintaticos.append("Erro: Uma '{' era esperado, linha %d" % atual[2])
            controle = 7
            funcao_declaracao(atual,prox)
    ...
    # Verifica se o token atual eh um comando
        ...
        if (atual[0] in comandos_lista or atual[1] == 1):
            ...
            erros_sintaticos.append("Erro: Um comando era esperado, linha %d" % atual[2])
            tipo_producao[3] = 'funcao'
            pilha_comandos.append('funcao')
            controle = 7
...
# A mensagem de ERRO sera, por exemplo, 'Erro. Uma '(' era esperado, linha 10'
...
for t in tokens:
    ...
    # alem de que para nao exceder o tamanho da lista e dar erro
        ...
        if t[0] == '}' and tokens[j+1][0] == '$':             # Essa eh a condicao correta, se entrar nela CHAMA TUDO corretamente
            ...
            erros_sintaticos.append("Erro: Final inesperado do programa, linha %d" % t[2])    # entrando nela imprimir mensagem de erro
...
# Antes o nome do arquivo era 'tokens.txt', com a analise lexica mudou
# Abrindo arquivo para armazenar a Lista de Tokens
arquivo = open('resultado.txt', 'w')

# Armazenando a Lista de Erros Lexicos, se existirem
if erro_geral:
    arquivo.write('Foram encontrados alguns Erros Lexicos:\n')
...
if len(erros_sintaticos) > 0:
    arquivo.write('Foram encontrados alguns Erros Sintaticos:\n')

    for linha in erros_sintaticos:
        arquivo.write('* ')
        arquivo.write('%s' % linha)
        arquivo.write('\n')
# Senao eh armazenada a mensagem de sucesso
else:
    arquivo.write('Analise Sintatica realizada com sucesso!\n')
